 ...
 * noble does. Records without it are accepted too, so a caller that has
 * already stripped it does not get rejected.
 *
 * @param {Buffer|null|undefined} manufacturerData
 * @returns {Buffer|null} The 9-byte PowerView record, or null if this is not one.
 */
function extractShadeRecord(manufacturerData) {
  if (!Buffer.isBuffer(manufacturerData)) return null;

  if (
    manufacturerData.length === ADVERTISEMENT_RECORD_LENGTH + 2
    && manufacturerData.readUInt16LE(0) === MANUFACTURER_ID
  ) {
    return manufacturerData.subarray(2);
  }
  if (manufacturerData.length === ADVERTISEMENT_RECORD_LENGTH) {
