...
function getHomeyCapabilities(typeId) {
  ...
  if (!capabilities.tiltOnly) {
    // A one-tap open/close alongside the slider. Listed first so it leads the
    // tile, and so it is the capability a quick action would attach to.
    result.push('powerview_open');
    result.push('powerview_status');
