...
class PowerViewShade {
  ...
  /**
   * @returns {boolean} Whether commands can be sent. An encrypted shade
   *   without a key silently ignores everything, so this is worth checking
   *   before promising the user anything.
   */
  get canControl() {
    return !this.encrypted || Boolean(this.homeKey);
  }
  ...
  async _transact(opcode, payload) {
    ...
    if (this.encrypted && !this.homeKey) {
      throw new ShadeError(
        'This shade is paired to a PowerView home and needs its home key before it will accept commands',
        'missing_home_key',
      );
    }
    ...
    if (this.encrypted) {
      frame = transformFrame(frame, this.homeKey);
    }
  ...
  _onNotification(data) {
    ...
    if (this.encrypted && this.homeKey) {
      try {
        plain = transformFrame(data, this.homeKey);
      } catch (err) {
   ...
   */
  async _pushClock() {
    if (this.encrypted && !this.homeKey) return; // would put plaintext on the wire
